...
class Medgan(object):
    ...
    def buildGenerator(self, x_input, z_input, bn_train):
        ...
        embeddings = tf.Variable(tf.random_uniform([2, tempDim], -1.0, 1.0))
        embed = tf.nn.embedding_lookup(embeddings, z_input)
        tempVec = tf.multiply(tempVec, embed)
    ...
    def getDiscriminatorResults(self, x_input, z_bool, keepRate, reuse=False):
        ...
        with tf.variable_scope('discriminator', reuse=reuse, regularizer=l2_regularizer(self.l2scale)):
            for i, discDim in enumerate(self.discriminatorDims[:-1]):
                ...
                tempVec = h
                tempDim = discDim
            W0 = tf.get_variable('W0', shape=[tempDim, 1])
            b0 = tf.get_variable('b0', shape=[1])
            W1 = tf.get_variable('W1', shape=[tempDim, 1])
            b1 = tf.get_variable('b1', shape=[1])
            y_hat = tf.where (z_bool,tf.squeeze(tf.nn.sigmoid(tf.add(tf.matmul(tempVec, W1), b1))),tf.squeeze(tf.nn.sigmoid(tf.add(tf.matmul(tempVec, W0), b0))))
            W_z = tf.get_variable('W_z', shape=[tempDim, 1])
            b_z = tf.get_variable('b_z', shape=[1])
            z_hat = tf.squeeze(tf.nn.sigmoid(tf.add(tf.matmul(tempVec, W_z), b_z)))
        return y_hat, z_hat
    ...
    def buildDiscriminator(self, x_real, z_real, x_fake, z_fake, zb_real, zb_fake, keepRate, decodeVariables, bn_train):
        ...
        loss_d_z = -tf.reduce_mean(z_real * tf.log(z_hat_real + 1e-12) + z_fake * tf.log(z_hat_fake + 1e-12)) - tf.reduce_mean((1. - z_real) * tf.log(1. - z_hat_real + 1e-12) + (1. - z_fake) * tf.log( 1. - z_hat_fake + 1e-12))
        loss_g_z = -tf.reduce_mean((1. - z_fake) * tf.log(z_hat_fake + 1e-12)) - tf.reduce_mean(z_fake * tf.log( 1. - z_hat_fake + 1e-12))
        loss_d_total = -tf.reduce_mean(tf.log(y_hat_real + 1e-12) + tf.log(1. - y_hat_fake + 1e-12) +
                                       lambda_fair* ( z_real * tf.log(z_hat_real + 1e-12) + z_fake * tf.log(z_hat_fake + 1e-12) +
            (1. - z_real) * tf.log(1. - z_hat_real + 1e-12) + (1. - z_fake) * tf.log(1. - z_hat_fake + 1e-12)))
        loss_g_total = -tf.reduce_mean( lambda_fair*( (1. - z_fake) * tf.log(z_hat_fake + 1e-12) + z_fake * tf.log(1. - z_hat_fake + 1e-12) )  + tf.log(y_hat_fake + 1e-12) )
    ...
    def generateData(self,
        ...
        z_random_idx = tf.placeholder('int32', [None])
    ...
    def train(self,
        ...
        x_random= tf.placeholder('float', [None, self.randomDim])
        z_raw = tf.placeholder('float', [None])
        z_raw_bool = tf.placeholder('bool', [None])
        z_random = tf.placeholder('float', [None])
        z_random_idx = tf.placeholder('int32', [None])
        z_random_bool = tf.placeholder('bool', [None])
        ...
        optimize_d_fair = tf.train.AdamOptimizer().minimize(loss_d_total + sum(all_regs), var_list=d_vars)
        optimize_g_fair = tf.train.AdamOptimizer().minimize(loss_g_total + sum(all_regs),
